...
bool SSE::handleBranch(const IntraCFGEdge* edge){
    ...
    std::string condstring = condition->toString();
    expr cond = getCtx().string_val(condstring);
    SVF::s64_t succ = edge->getSuccessorCondValue();
    int intsuccessorVal = (int)succ;
    expr successorVal = getCtx().int_val(intsuccessorVal);
